...
your_model = tf.keras.models.load_model(PREDICCION_CC_DIR / 'my_model_completo.h5')
...
def extract_combined_features(audio_path):
    ...
    combined_features = np.concatenate([vggish_features.flatten(), mfcc_features])
...
def extract_additional_features(audio_path):
    ...
    return np.concatenate([zcr, spectral_centroid, spectral_rolloff], axis=0).flatten()

# Prueba con un archivo de audio específico
audio_file_path = str(PREDICCION_CC_DIR / 'Audios' / 'Train' / 'Placa_12mm' / 'E7018' / 'DC' / '240926-144305_Audio' / 'drums_louder.wav')

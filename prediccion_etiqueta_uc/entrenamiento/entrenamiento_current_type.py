...
# Cargar los datos
print("=" * 60)
print("ENTRENAMIENTO - TYPE OF CURRENT")
print("=" * 60)
print("\n>> Cargando datos desde rutas_etiquetas_current_type.csv...")
script_dir = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(script_dir, "..", "rutas_etiquetas_current_type.csv")
...
def convertir_caracteristicas_a_numeros(data):
    ...
    for col in data.columns:
        if isinstance(data[col].iloc[0], str) and data[col].iloc[0].startswith(
            ...
            data[col] = data[col].apply(lambda x: x.flatten())  # Aplana el array
...
X = data.drop(columns=["Audio Path", "Type of Current"]).to_numpy(dtype=np.float32)
...
model_path = os.path.join(uc_dir, "my_model_completo03.keras")
model.save(model_path)
print("[OK] Entrenamiento finalizado exitosamente")
print("    Modelo guardado como: my_model_completo03.keras")
